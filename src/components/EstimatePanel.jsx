...
const EstimatePanel = () => {
  ...
  const [newRecord, setNewRecord] = useState({
    head: '',
    program: '0', // Default to 0
    project: '',
    sub_project: '0', // Default to 0
    object: '',
    revenue_code_name: '',
    estimate: '',
    re_estimate: ''
  ...
  const handleAddRecord = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        toast.error(error.response?.data?.message || 'Failed to add estimate');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    // if (!confirm(`Are you sure you want to delete ${selectedRows.length} selected record(s)? This action cannot be undone.`)) {
    //   return;
    // }

    setLoading(true);
    try {
      // Send IDs as comma-separated query parameters
      const idsParam = selectedRows.join(',');
      const response = await axios.delete(`${API_BASE_URL}/estimates/delete-multiple?ids=${idsParam}`, {
        headers: getAuthHeaders()
      });

      if (response.data.success) {
        toast.success(`Successfully deleted ${selectedRows.length} record(s)`);
        setSelectedRows([]);
        fetchRecords();
        fetchFilterOptions();
        fetchSummary();
      ...
      if (error.response?.status === 401) {
        ...
        toast.error('Some records were not found. Refreshing the list...');
        setSelectedRows([]);
        fetchRecords();
      } else if (error.response?.status === 422) {
        const errors = error.response.data.errors;
        ...
        toast.error(error.response?.data?.message || 'Failed to delete records. Please try again.');
      }
    } finally {
      setLoading(false);
    }
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left font-semibold text-gray-700">Revenue Name</th>
          ...
          <div className="fixed inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                {/*  Combined Row */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Revenue Code</label>
                  <div className="flex gap-1.5">
                    <input
                      ...
                      min="0"
                      step="1"
                      className="w-1/3 border rounded-lg px-2 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent appearance-none [-moz-appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                      placeholder="1002"
                    />
                    <span className="self-center text-gray-400 font-bold text-sm">-</span>
                    <input
                      type="number"
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Format: 1004 - 04 - 03</p>
                </div>
                ...
                <div className="hidden">
                  <input type="hidden" value={newRecord.program} />
                  <input type="hidden" value={newRecord.sub_project} />
                </div>
              </div>
