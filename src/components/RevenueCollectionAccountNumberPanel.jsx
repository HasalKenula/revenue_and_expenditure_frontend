...
const RevenueCollectionAccountNumber = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await apiClient.get('/revenue-collection-account-number/data', { params });
      ...
      if (response.data.success) {
        ...
        setAccountsWithData(data.accounts_with_data || 0);
  ...
  const fetchFilterOptions = async () => {
    try {
      const response = await apiClient.get('/revenue-collection-account-number/filter-options');
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      // ==================== PAGE 2: SECOND TABLE (Jul to Dec with Total = ALL 12 MONTHS) ====================
      // Add a new page
      doc.addPage();
      ...
      // Second Table: Jul to Dec (Months 7-12) + Total (Sum of ALL 12 months)
      const secondTableHeaders = ['Account Number', 'Revenue Code', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec', 'Total (All 12 Months)'];
      const secondMonthKeys = ['7', '8', '9', '10', '11', '12'];
...
export default RevenueCollectionAccountNumber;