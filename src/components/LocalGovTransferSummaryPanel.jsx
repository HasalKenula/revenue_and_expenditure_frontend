...
const LocalGovTransferSummaryPanel = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await apiClient.get('/local-gov-transfer-summary/data', { params });

      if (response.data.success) {
        const records = response.data.data.local_gov_transfer_summary || [];
        ...
        records.forEach(record => {
          ...
          if (record.month === 'December' || record.month === 'TOTAL') {
            cumulativeTotal = record.cumulative_expenditure || 0;
          }
        });

        // If December doesn't exist, get the last month's cumulative
        if (cumulativeTotal === 0 && records.length > 0) {
          const lastRecord = records[records.length - 2];
          if (lastRecord) {
            cumulativeTotal = lastRecord.cumulative_expenditure || 0;
          }
        }
  ...
  const fetchFilterOptions = async () => {
    try {
      const response = await apiClient.get('/local-gov-transfer-summary/filter-options');
  ...
  const clearFilters = () => {
    ...
    setData([]);
    setTotals({ total_amount: 0, cumulative_total: 0 });
    setCurrentPage(1);
    setTotalRecords(0);
    setLastPage(1);
  };
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      doc.text('Local Government Transfer Summary Report', pageWidth / 2, 20, { align: 'center' });
      ...
      // Table headers
      const tableHeaders = ['Month', 'Total', 'Cumulative Expenditure'];
      ...
      const tableBody = data.map(record => {
        return [
          ...
          formatNumber(record.cumulative_expenditure || 0)
        ];
      });
      ...
      autoTable(doc, {
        ...
        margin: { top: 45, left: 30, right: 30, bottom: 20 },
        tableWidth: 180,
        didParseCell: function (data) {
          ...
          // Highlight Cumulative Expenditure column
          if (data.row.index === data.table.body.length - 1) {
            data.cell.styles.fontStyle = 'bold';
      ...
      const fileName = `local_gov_transfer_summary_${appliedFilters.year}.pdf`;
      doc.save(fileName);
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const response = await apiClient.get('/local-gov-transfer-summary/export', { params });
      ...
      if (response.data.success) {
        ...
        if (csvData.length > 0) {
          ...
          a.href = url;
          a.download = `local_gov_transfer_summary_${appliedFilters.year}.csv`;
          a.click();
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Local Government Transfer Summary Report</h1>
              <p className="text-sm text-gray-500 mt-1">
                Summary of Local Government Transfers with cumulative expenditure
              </p>
            </div>
            {appliedFilters.year && (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-4 py-3 text-left font-semibold text-gray-700 border border-gray-300 sticky left-0 bg-gray-50">Month</th>
                  <th className="px-4 py-3 text-right font-semibold text-gray-700 border border-gray-300">Total(Rs)</th>
                  <th className="px-4 py-3 text-right font-semibold text-gray-700  border border-gray-300">Cumulative Expenditure(Rs)</th>
                </tr>
              </thead>
              <tbody>
                {!appliedFilters.year ? (
                  ...
                  paginatedData.map((record, index) => {
                    ...
                    return (
                      ...
                      >
                        <td className={`px-4 py-3 sticky left-0 bg-white border border-gray-300 ${isTotal ? 'bg-gray-100' : ''}`}>
                          {record.month || '-'}
                        </td>
                        <td className="px-4 py-3 text-right text-gray-600 border border-gray-300">
                          {formatNumber(record.total || 0)}
                        </td>
                        <td className={`px-4 py-3 text-right font-bold border border-gray-300 ${isTotal ? 'text-gray-700' : 'text-gray-600'
                          }`}>
                          {formatNumber(record.cumulative_expenditure || 0)}
                        </td>
                      </tr>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 rounded-lg p-3">
                  <p className="text-xs text-blue-700">
                    <strong>Note:</strong> This report shows summary of Local Government Transfers.
                  </p>
                  <p className="text-xs text-blue-700 mt-1">
                    <strong>Total = Salary Reimbursement + Members Allowances</strong>
                  </p>
                  <p className="text-xs text-blue-700 mt-1">
                  ...
                  <p className="text-xs text-blue-700 mt-1">
                    <strong>Cumulative Expenditure:</strong> Running total from January to current month
                  </p>
                </div>
              </div>
...
export default LocalGovTransferSummaryPanel;