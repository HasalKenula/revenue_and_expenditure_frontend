...
const ODSPanel = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await apiClient.get('/ods/data', { params });
  ...
  const fetchFilterOptions = async () => {
    try {
      const response = await apiClient.get('/ods/filter-options');
  ...
  const clearFilters = () => {
    ...
    setRecords([]);
    setTotals({ total_surcharge: 0, total_records: 0 });
    setCurrentPage(1);
    setTotalRecords(0);
    setLastPage(1);
  };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Other Department Surcharge</h1>
              <p className="text-sm text-gray-500 mt-1">
                CR transactions where TR No is different from Head (CR Code: 2000)
              </p>
            </div>
        ...
        <div className="bg-amber-50 rounded-lg p-3 border border-amber-200">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-amber-700">
              <strong>Note:</strong> Showing records where <strong>TR No ≠ Head</strong> and <strong>CR Code = 2000</strong> (Surcharge)
            </span>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs border-collapse">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-2 py-3 text-left font-semibold text-gray-700 border border-gray-300 sticky left-0 bg-gray-50">TR No</th>
                  <th className="px-2 py-3 text-left font-semibold text-gray-700 border border-gray-300">Head</th>
                  <th className="px-2 py-3 text-left font-semibold text-gray-700 border border-gray-300">Program</th>
                  <th className="px-2 py-3 text-left font-semibold text-gray-700 border border-gray-300">Project</th>
                  <th className="px-2 py-3 text-left font-semibold text-gray-700 border border-gray-300">Object</th>
                  <th className="px-2 py-3 text-left font-semibold text-gray-700 border border-gray-300">Sub Project</th>
                  <th className="px-2 py-3 text-left font-semibold text-gray-700 border border-gray-300">Sub Object</th>
                  <th className="px-2 py-3 text-right font-semibold text-gray-700 border border-gray-300">Surcharge Amount (Rs)</th>
                </tr>
              </thead>
              <tbody>
                {!appliedFilters.year || !appliedFilters.month ? (
                  ...
                  paginatedRecords.map((record, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition">
                      <td className="px-2 py-2 font-medium text-gray-900 sticky left-0 bg-white border border-gray-300">{record.trno}</td>
                      <td className="px-2 py-2 text-gray-700 border border-gray-300">{record.head}</td>
                      <td className="px-2 py-2 text-gray-700 border border-gray-300">{record.program}</td>
                      <td className="px-2 py-2 text-gray-700 border border-gray-300">{record.project}</td>
                      <td className="px-2 py-2 text-gray-700 border border-gray-300">{record.object}</td>
                      <td className="px-2 py-2 text-gray-700 border border-gray-300">{record.sub_project}</td>
                      ...
                      <td className="px-2 py-2 text-right font-medium text-gray-600 border border-gray-300">
                        Rs{formatNumber(record.surcharge_amount)}
                      </td>
                    </tr>
                  ))
                )}
              ...
              {paginatedRecords.length > 0 && (
                <tfoot className="bg-gray-50 border-t border-gray-200">
                  <tr className="font-semibold">
                    <td colSpan="7" className="px-2 py-3 text-right border border-gray-300">Total:</td>
                    <td className="px-2 py-3 text-right text-gray-600 border border-gray-300">
                      Rs{formatNumber(totals.total_surcharge)}
                    </td>
                  </tr>
                </tfoot>
              )}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 rounded-lg p-3">
                  <p className="text-xs text-blue-700">
                    <strong>Note:</strong> This report shows CR transactions where:
                  </p>
                  <ul className="text-xs text-blue-700 mt-1 list-disc list-inside">
                    <li>CR Code = 2000 (Surcharge)</li>
