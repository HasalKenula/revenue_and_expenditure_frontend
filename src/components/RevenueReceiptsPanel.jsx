...
const RevenueReceipts = () => {
  ...
  const fetchReceipts = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        alert('Failed to fetch revenue receipts');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleAddReceipt = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        alert('Revenue receipt added successfully!');
        setNewReceipt({ 
          account_number_id: '', 
          amount: '', 
          month: '',
      ...
      if (error.response?.status === 401) {
        ...
        alert(error.response?.data?.message || 'Failed to add revenue receipt');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdateReceipt = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        alert('Revenue receipt updated successfully!');
      ...
      if (error.response?.status === 401) {
        ...
        alert(error.response?.data?.message || 'Failed to update revenue receipt');
      }
    } finally {
      setLoading(false);
    }
