...
const ImpressSettlementPanel = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/impress-settlement/records`, {
        params,
  ...
  const fetchSummary = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/impress-settlement/summary`, {
        headers: getAuthHeaders()
      });
      if (response.data.success) {
  ...
  const fetchFilterOptions = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/impress-settlement/filter-options`, {
        headers: getAuthHeaders()
      });
      if (response.data.success) {
  ...
  const handleImport = async () => {
    ...
    setUploading(true);
    try {
      const response = await axios.post(`${API_BASE_URL}/impress-settlement/import`, formData, {
        headers: getFileUploadHeaders()
      });

      if (response.data.success) {
        toast.success(`Import successful! Imported: ${response.data.imported_count} records`);
  ...
  const handleAddRecord = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${API_BASE_URL}/impress-settlement/records`, {
  ...
  const handleUpdateRecord = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.put(`${API_BASE_URL}/impress-settlement/records/${editingRecord.id}`, {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.delete(`${API_BASE_URL}/impress-settlement/records`, {
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage impress settlements and reimbursements</p>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <button onClick={() => setShowAddModal(true)} className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm">
            <Plus size={16} /><span>Add Settlement</span>
          </button>
                ...
                {records.length === 0 ? (
                  <tr>
                    <td colSpan="8" className="text-center py-8 text-gray-500">
                      No records found. Add a settlement record or import Excel.
                    </td>
                  </tr>
                ) : (
                  records.map((record) => (
        ...
        {showAddModal && (
          <div className="fixed inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-xl w-full max-w-md p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Add Impress Settlement</h3>
        ...
        {showEditModal && editingRecord && (
          <div className="fixed inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white rounded-xl w-full max-w-md p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">Edit Impress Settlement</h3>
...
export default ImpressSettlementPanel;