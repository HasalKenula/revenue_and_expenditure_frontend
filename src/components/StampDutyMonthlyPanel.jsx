...
const StampDutyMonthlyPanel = () => {
    ...
    //       // Build table headers
    //       const tableHeaders = ['TR No', 'Program', 'Project', 'Sub Project', 'Object', 'Subject Name', 'Allocation'];
    ...
    //       tableHeaders.push('Total Expenditure', 'Balance');
    ...
    //       const totalBaseWidth = 7 * baseWidth; // 7 base columns
    //       const totalMonthWidth = months.length * monthWidth;
    //       const remainingWidth = availableWidth - totalBaseWidth - totalMonthWidth;
    //       const extraWidth = remainingWidth / totalCols;

    //       const columnStyles = {};
    //       tableHeaders.forEach((_, index) => {
    //         if (index < 7) {
    //           columnStyles[index] = { 
    //             cellWidth: baseWidth + extraWidth, 
    //             halign: index === 0 ? 'center' : index === 5 ? 'left' : 'right' 
    //           };
    //         } else if (index < 7 + months.length) {
    //           columnStyles[index] = { cellWidth: monthWidth + extraWidth, halign: 'right' };
    //         } else {
    //           columnStyles[index] = { cellWidth: baseWidth + extraWidth, halign: 'right' };
    //         }
    //       });
    ...
    //           }
    //           // Highlight the last two columns (Total Expenditure and Balance)
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                        <thead className="bg-gray-50 border-b border-gray-200">
                            <tr>
                                <th className="px-2 py-2 text-left font-semibold text-gray-700 sticky left-0 bg-gray-50">Head</th>
                                <th className="px-2 py-2 text-left font-semibold text-gray-700">Program</th>
                                <th className="px-2 py-2 text-left font-semibold text-gray-700">Project</th>
                                ...
                                <th className="px-2 py-2 text-left font-semibold text-gray-700">Subject Name</th>
                                ...
                                <th className="px-2 py-2 text-right font-semibold text-gray-700 bg-blue-50">Total Exp.</th>
                                <th className="px-2 py-2 text-right font-semibold text-gray-700 bg-purple-50">Balance</th>
                            </tr>
                        </thead>
                        <tbody>
                            {!appliedFilters.year || !appliedFilters.month ? (
                                ...
                                paginatedData.map((record, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className={`px-2 py-2 sticky left-0 bg-white ${isTotal ? 'bg-gray-100' : ''}`}>
                                                {record.trno || '-'}
                                            </td>
                                            ...
                                            <td className="px-2 py-2 max-w-[100px] truncate" title={record.subject_name}>
                                                {record.subject_name || '-'}
                                            </td>
                                            <td className="px-2 py-2 text-right text-blue-600">Rs{formatNumber(record.allocation)}</td>
                                            {months.map(month => {
                                                const key = `month_${month}`;
                                            ...
                                            <td className="px-2 py-2 text-right font-bold text-green-700 bg-blue-50">
                                                Rs{formatNumber(record.total_expenditure || 0)}
                                            </td>
                                            <td className={`px-2 py-2 text-right font-bold text-purple-700 bg-purple-50 ${parseFloat(record.balance) < 0 ? 'text-red-600' : ''
                                                }`}>
