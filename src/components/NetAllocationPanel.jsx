...
const NetAllocationPanel = () => {
    ...
    // Check if view should be object-wise
    ...
    const handleExportPDF = () => {
        ...
        try {
            // Create PDF in portrait orientation (A4 vertical)
            const doc = new jsPDF({
            ...
            doc.text('Net Allocation Report', doc.internal.pageSize.getWidth() / 2, 15, { align: 'center' });
            ...
            let filterText = '';
            if (appliedFilters.head) filterText += `TR No: ${appliedFilters.head} | `;
            ...
            const isObjectWise = isObjectWiseView;
            ...
            const columnStyles = isObjectWise ? {
                0: { cellWidth: 30 },
            ...
            // Save PDF
            const fileName = `net_allocation_report_${new Date().toISOString().split('T')[0]}.pdf`;
            doc.save(fileName);
        ...
        try {
            ...
            if (response.data.success) {
                ...
                if (csvData && csvData.length > 0) {
                    ...
                    // Create CSV rows with proper handling of zero values
                    const csvRows = [
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm border-collapse">
                            <thead className="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    <th className="px-4 py-3 text-left font-semibold text-gray-700 border border-gray-300">Object Code</th>
                                    {!isObjectWiseView && (
                                        <th className="px-4 py-3 text-left font-semibold text-gray-700 border border-gray-300">Sub Project</th>
                                    )}
                                    <th className="px-4 py-3 text-right font-semibold text-gray-700 border border-gray-300">Allocation (Rs)</th>
                                    <th className="px-4 py-3 text-right font-semibold text-gray-700 border border-gray-300">FR 30 P (Rs)</th>
                                    <th className="px-4 py-3 text-right font-semibold text-gray-700 border border-gray-300">FR 30 M (Rs)</th>
                                    <th className="px-4 py-3 text-right font-semibold text-gray-700 border border-gray-300">Supplementary (Rs)</th>
                                    <th className="px-4 py-3 text-right font-semibold text-gray-700 border border-gray-300">Net Allocation (Rs)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {!appliedFilters.head && !appliedFilters.month ? (
                                    <tr>
                                        <td colSpan={isObjectWiseView ? 6 : 7} className="text-center py-12 text-gray-500">
                                    ...
                                    paginatedRecords.map((record, index) => (
                                        <tr key={index} className="border-b border-gray-100 hover:bg-gray-50 transition">
                                            <td className="px-4 py-3 font-medium text-gray-900 border border-gray-300">{record.object}</td>
                                            {!isObjectWiseView && (
                                                <td className="px-4 py-3 text-gray-600 border border-gray-300">{record.subproject}</td>
                                            )}
                                            <td className="px-4 py-3 text-right text-gray-900 border border-gray-300">{formatNumber(record.allocation)}</td>
                                            <td className="px-4 py-3 text-right text-gray-600 font-medium border border-gray-300">{formatNumber(record.fr66p)}</td>
                                            <td className="px-4 py-3 text-right text-gray-600 font-medium border border-gray-300">{formatNumber(record.fr66m)}</td>
                                            <td className="px-4 py-3 text-right text-gray-600 font-medium border border-gray-300">{formatNumber(record.supplementary)}</td>
                                            <td className="px-4 py-3 text-right font-bold text-gray-600 border border-gray-300">{formatNumber(record.net_allocation)}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                            {paginatedRecords.length > 0 && (
                                <tfoot className="bg-gray-50 border-t border-gray-200">
                                    <tr className="font-semibold">
                                        <td colSpan={isObjectWiseView ? 1 : 2} className="px-4 py-3 text-right border border-gray-300">Total:</td>
                                        <td className="px-4 py-3 text-right text-gray-600 border border-gray-300">{formatNumber(totals.total_allocation)}</td>
                                        <td className="px-4 py-3 text-right text-gray-600 border border-gray-300">{formatNumber(totals.total_fr66p)}</td>
                                        <td className="px-4 py-3 text-right text-gray-600 border border-gray-300">{formatNumber(totals.total_fr66m)}</td>
                                        <td className="px-4 py-3 text-right text-gray-600 border border-gray-300">{formatNumber(totals.total_supplementary)}</td>
                                        <td className="px-4 py-3 text-right font-bold text-gray-600 border border-gray-300">{formatNumber(totals.total_net_allocation)}</td>
                                    </tr>
                                </tfoot>
                            )}
                        </table>
