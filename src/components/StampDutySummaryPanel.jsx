...
const StampDutySummaryPanel = () => {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await apiClient.get('/stamp-duty-summary/data', { params });

      if (response.data.success) {
        const data = response.data.data.stamp_duty_summary || [];
        
        setStampDutyData(data);
        setSelectedYear(response.data.data.filters?.year || '');
        setSelectedMonth(response.data.data.filters?.month || '');

        // Calculate totals (excluding the TOTAL row)
  ...
  const fetchFilterOptions = async () => {
    try {
      const response = await apiClient.get('/stamp-duty-summary/filter-options');
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      doc.text('Stamp Duty Summary Report', pageWidth / 2, 20, { align: 'center' });
      ...
      // Table headers - only required columns
      const tableHeaders = ['Head', 'Program', 'Project', 'Sub Project', 'Object', 'Subject Name', 'Allocation', 'Total Expenditure', 'Balance'];
      ...
      const tableBody = stampDutyData.map(record => {
        return [
          ...
          formatNumber(record.total_expenditure || 0),
      ...
      autoTable(doc, {
        ...
        didParseCell: function(data) {
          ...
          // Highlight Total Expenditure and Balance columns
      ...
      const fileName = `stamp_duty_summary_${appliedFilters.year}_${monthText}.pdf`;
      doc.save(fileName);
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`px-3 py-2 text-right font-bold text-purple-700 bg-purple-50 ${
                        parseFloat(record.balance) < 0 ? 'text-red-600' : ''
                      }`}>
...
export default StampDutySummaryPanel;