...
const BudgetPanel = ({ onSelectDepartment }) => {
    ...
    const fetchBudgetRecords = async () => {
        ...
        try {
            ...
            if (error.response?.status === 401) {
                ...
                toast.error('Failed to fetch budget records. Please check your API connection.');
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleAddRecord = async () => {
        if (!newRecord.objname || !newRecord.amount) {
            toast.error('Please fill all required fields (Name and Amount)');
            return;
        }
    ...
    const handleFileUpload = async (file) => {
        ...
        try {
            ...
            if (error.response?.status === 401) {
                ...
                toast.error(error.response?.data?.message || 'Failed to import file. Please check the file format.');
            }
        } finally {
            setUploading(false);
        }
